...
40 fixtures total today: 28 paragraph positives, 4 paragraph negatives, 8 document positives. Coverage spans all 20 rules in the catalog.

## Baseline

Run against the live Anthropic stack (Haiku 4.5 for paragraph, Sonnet 4.6 for document):

- **Overall pass rate**: 40/40 (100%) as of the last commit.
- Paragraph mean latency: ~9–10s (the eval runs serially without prompt-cache warming; production p50 is faster).
- Document mean latency: ~9s.

Per-rule pass rate is printed by the runner each invocation. Anything below 100% is a regression — open an issue or commit a prompt/rule fix.

> A previous experiment routed Pass B through Inception Labs' Mercury (diffusion) as a "hybrid" mode. It's been removed for now — see `docs/RULES_AUDIT.md` for context. The provider abstraction in `lib/analyze/providers/` is kept so a second backend can be re-plugged with minimal churn.

## All 40 fixtures, annotated
